...
{
    internal class DrawingMagnifier : IDrawable<(int x, int y)>
    {
        ...
        public ScreenshotState ScreenshotState { get; set; }
        ...
        public DrawingMagnifier(
            ScreenshotState screenshotState,
            MonitorInfo monitorInfo,
            Func<int, int, Color> colorGetter)
        {
            ScreenshotState = screenshotState;
            MonitorInfo = monitorInfo;
        ...
        public bool CanDraw((int x, int y) drawingData)
        {
            if (ScreenshotState.Mode == ScreenshotMode.Fixed &&
                ScreenshotState.Orientation != PointOrientation.Center) return false;

            var (physicalX, physicalY) = drawingData;

            return MonitorInfo.PhysicalScreenRect.Contains(physicalX, physicalY);
        }

        public void Draw(DrawingContext dc, (int x, int y) drawingData)
        {
            var (physicalX, physicalY) = drawingData;
